...
REM .conf files.
if exist "Variables.conf" (
    for /f "usebackq eol=# tokens=1,2 delims==" %%A in ("Variables.conf") do set "%%A=%%~B"
)

if not exist "Git-Push_Info.conf" (
...
set "ChoiceOptions="
set "DisplayOptions="
for /f "usebackq eol=# tokens=1,2 delims==" %%A in ("Git-Push_Info.conf") do (
    set "Key=%%A"
    set "Rest=%%B"
